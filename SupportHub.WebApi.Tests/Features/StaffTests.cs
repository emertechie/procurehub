    ...
    : TestsBase(testOutputHelper)
{
    ...
    {
        // Assert no Staff yet
        var listDeptsResp1 = await Client.GetAsync("/staff", CancellationToken);
        ...
        var listDeptsResp2 = await Client.GetAsync("/staff", CancellationToken);
        ...
        var getStaffResp = await Client.GetAsync($"/staff/{newStaffId}", CancellationToken);
